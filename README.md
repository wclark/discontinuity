...
The current native prototype is in [`unity/`](unity/README.md), using Unity 6000.6.0f1. Play one incarnation at a time in an illustrated, choice-driven story and author new actions in the same interface. Actions show their scores in descending order. The info button exposes availability gates, scoring conditions, effects and recorded increments. Each manual choice records the score gap plus one, including +1 for an already-leading or tied choice; increments never compound. The action editor saves definitions locally, and portable state records preserve the world, content and playthrough for further iteration. Movement remains simultaneous, crossings are witnessed, and two complete interaction illustrations start an exact-match scene library.
