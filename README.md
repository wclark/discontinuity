...
The current native prototype is in [`unity/`](unity/README.md), using Unity 6000.6.0f1. **Adjustment mode** is a story-composing interface: actions are ranked by score, and your weights remain active when you revisit a character. Accepting the first-ranked choice adds nothing; choosing another adds only the gap plus one. Availability uses location, time, possessions, people present and items present. Each turn shows witnessed results beside the next choices. Opposite travelers get one midpoint reaction beat before arriving. Author actions and crossing reactions in the same editor, follow top choices passively, and export complete weight sets or exact state records. The illustrated scenes and branded desktop launcher remain in place.
