...
Everyone chooses against the same start-of-turn state. The player chooses manually; NPCs use the same valid options and conditional scores. Resolve conversation, then room activity, then a simultaneous movement batch, then ordinary waiting. Adjustment mode adds one bounded midpoint opportunity when travelers cross on the same edge. It does not rerun anyone's ordinary action.
...
| --- | --- | --- |
| Clara leaves the Hall for the Archive; Jonah comes the other way | They meet at the midpoint and each chooses one reaction before completing the move | Keep going, greet, offer a carried item, or an authored reaction; the next full turn begins after arrival |
...
| A third traveler shares a crossing edge | All travelers share one midpoint, with one reaction each rather than one per pair | A two-person illustration cannot match this three-person encounter |
...
### The Crossing Half-Turn

1. Resolve room actions once, then validate and commit travel intents.
2. Opposite travelers meet at the midpoint of the existing fifteen-minute interval, e.g. 08:07:30. Their logical location is the canonical graph edge, such as `edge:archive:hall`. Bystanders in either room are not present in that passage.
3. Rank every traveler's reactions from the same midpoint state. Keep going starts at zero, like every option; conditional and persistent manual weights apply normally. A carried object is present in the passage, but a loose item in an adjacent room is not.
4. Resolve one reaction per traveler using the rotating actor priority. Revalidate requirements. No counter-reaction loop is allowed. NPC-only encounters resolve without pausing an absent player. The player can create a new reaction while the crossing is pending.
5. Finish the original journeys and remaining end-of-turn actions, then advance to 08:15. Reactions currently do not cancel or reroute those journeys; any resulting change of plan is evaluated at the next normal decision.

`World.transit` persists committed choices, original rankings, snapshots, and crossing participants. Reloading a midpoint resumes only its reaction phase, without replaying earlier room activity. Crossing snapshots include committed endpoints for direction/caption rendering, not as a claim that arrival already happened. The engine gives all actual arrivals one shared final snapshot.

This intentionally leaves real-time speed, mid-edge stopping and recursive interruptions out. Legacy story-mode scene-discovery fixtures retain the previous no-reaction behavior; the default adjustment interface uses the bounded half-turn described here.

## Household Tasks

`RoomLife` supplies ordinary authored actions linked by completion facts. Location, time, prerequisite completion and noncompletion conditions contribute one point. Stronger plot choices interrupt them naturally. Completed work stops receiving its conditional points. In adjustment mode it can remain physically available for a deliberate override; completion does not secretly hide it. The chains do not grant bonuses outside their conditions.
...
2. Inspect `artifacts/scene-catalog.json`. It records the sampling protocol, run count, unique signatures, illustrated count, frequency in the sample and a prompt for each signature. Current legacy story-mode sampling finds 417 signatures across 342 runs; two have complete illustrations. This is not exhaustive reachability, does not enumerate newly authored composing/crossing actions, and does not imply the other 415 images have been generated.
